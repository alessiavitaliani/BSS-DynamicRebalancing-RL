...
class Cell:
    ...
    def add_departure(self, d: int = 1) -> None:
        if 'total_departures' not in self._metrics: self._metrics['total_departures'] = 0
        if 'total_demand' not in self._metrics: self._metrics['total_demand'] = 0
            
        self._metrics['total_departures'] += d
        self._metrics['total_demand'] += d

    def add_failure(self, f: int = 1) -> None:
        if 'failures' not in self._metrics: self._metrics['failures'] = 0
        if 'total_demand' not in self._metrics: self._metrics['total_demand'] = 0
